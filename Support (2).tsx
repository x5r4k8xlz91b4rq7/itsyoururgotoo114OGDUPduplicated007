...
const Support = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm() || isSubmitting) {
      showNotification('error', t('common.pleaseFixErrors'));
      return;
    }
    ...
    try {
      ...
      if (selectedFiles.length > 0 && !isUploading) {
        await uploadFiles(selectedFiles, updatedMetadata);
      }
      ...
      if (dbResult.success || emailResult?.success) {
        let message = t('support.successMessage');
        ...
        if (!dbResult.success && emailResult?.success) {
          message += t('support.emailOnlySuccess');
        }
        ...
        showNotification('error', t('support.errorMessage'));
      }
    } catch (error) {
      showNotification('error', t('common.generalError'));
    } finally {
